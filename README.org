...
You can also trigger summarization from the right-click context menu — especially handy for links and selected text:

- GPT2Org-org-protocol :: Capture the current page URL or selected text and send
  it to Emacs via Org Protocol.
- GPT2Org-org-journal :: Capture the current page URL or selected text and send
  it to your Emacs journal via Org Protocol.
- GPT2Org-clock-in :: Capture the current page URL or selected text and clock it
  into Emacs via Org Protocol.
- Thinking Mode (DeepSeek) :: The extension supports DeepSeek's thinking mode
  using the thinking and reasoning_effort API parameters.
